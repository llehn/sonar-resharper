...
public class ReSharperSensor implements Sensor {
  ...
  // Since the ruleset differs between different versions of the Resharper commandline tool we need to ensure
  // that it runs with a new enough (or newer) version of the command line tool
  public static final String MINIMUM_TOOLS_VERSION = "102.0";
  ...
  void analyse(SensorContext context, FileProvider fileProvider, ReSharperDotSettingsWriter writer, ReSharperReportParser parser, ReSharperExecutor executor) {
    ...
    String toolsVersionString = parser.parseToolsVersion(reportFile);
    final int toolsVersion = Integer.parseInt( toolsVersionString.replaceAll( "\\.", "" ) );
    final int minimumToolsVersion = Integer.parseInt( MINIMUM_TOOLS_VERSION.replaceAll( "\\.", "" ) );

    if(toolsVersion < minimumToolsVersion) {
      throw new SonarException("The report generated by the Resharper Commandline utility reports ToolsVersion " + toolsVersionString + " whereas " + MINIMUM_TOOLS_VERSION + " is the minimum requirement. Please update the resharper tool and try again.");
    }
